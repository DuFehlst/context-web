...
test('reuses the live map iframe and retries initialization only after iframe load', async () => {
  const source = await readFile(new URL('../src/client/synapseBridge.ts', import.meta.url), 'utf8')
...
test('opens the clicked card in a tool-aware detail inspector', async () => {
  ...
  assert.match(inspector, /<svg aria-hidden="true" viewBox="0 0 24 24"/)
...
test('mirrors DSH theme changes into the map', async () => {
  const clientSource = await readFile(new URL('../src/client/synapseBridge.ts', import.meta.url), 'utf8')
...
test('renders a follow-up plus on final cards and fold plus branch controls on non-final cards', async () => {
  ...
  assert.match(card, /M20\.75 12C/)
  assert.match(card, /M11\.25 20C/)
  ...
  assert.match(card, /M16\.5 2\.25C14\.7051/)
