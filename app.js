...
// 会话地图 → Markdown 大纲：工作区 → 线程（按血缘缩进）→ 每条消息一行摘要。
// 纯函数，便于单测；下载动作单独放在 downloadTextFile。
const OUTLINE_MESSAGE_LIMIT = 160
const OUTLINE_KIND_LABELS = { user: '用户', assistant: '回答', error: '失败', todo: '待办' }

function outlineLine(message) {
  const text = String(message?.text ?? '').replaceAll(/\s+/g, ' ').trim()
  return text.length > OUTLINE_MESSAGE_LIMIT ? `${text.slice(0, OUTLINE_MESSAGE_LIMIT)}…` : text
}

function markdownOutline(workspace) {
  if (workspace === null || workspace === undefined) return ''
  const threads = Array.isArray(workspace.threads) ? workspace.threads : []
  const lines = [`# ${workspace.title ?? '会话地图'}`]
  if (typeof workspace.cwd === 'string' && workspace.cwd !== '') lines.push('', `> 工作目录：\`${workspace.cwd}\``)
  lines.push('', `> 共 ${threads.length} 条会话/分支 · 导出时间 ${new Date().toISOString()}`)
  const byParent = new Map()
  for (const thread of threads) {
    const parent = typeof thread.parentId === 'string' && threads.some(item => item.id === thread.parentId) ? thread.parentId : null
    byParent.set(parent, [...(byParent.get(parent) ?? []), thread])
  }
  const seen = new Set()
  const walk = (thread, depth) => {
    if (seen.has(thread.id)) return
    seen.add(thread.id)
    lines.push('', `${'#'.repeat(Math.min(6, depth + 2))} ${thread.title ?? '未命名会话'}`)
    const messages = Array.isArray(thread.messages) ? thread.messages : []
    if (messages.length === 0) lines.push('', '_（暂无投影消息）_')
    for (const message of messages) {
      const kind = OUTLINE_KIND_LABELS[message.kind] ?? message.kind ?? '消息'
      const tools = Array.isArray(message.process) && message.process.length > 0 ? `（工具 ${message.process.length} 次）` : ''
      const text = outlineLine(message)
      lines.push(`- **${kind}**${tools}${text === '' ? '' : `：${text}`}`)
    }
...
function outlineFileName(workspace) {
  const title = String(workspace?.title ?? 'context-web').replaceAll(/[\\/:*?"<>|\s]+/g, '-').replaceAll(/^-+|-+$/g, '').slice(0, 40) || 'context-web'
...
app.addEventListener('click', async event => {
  ...
  try {
    ...
    // Map → Agent 画布：切到该会话并选中内核的 Agent 画布标签；没有 DSH 会话的
    // 手工卡片只能留在画布上。
    if (button.dataset.action === 'open-canvas') {
      if (thread?.dshSessionId === null || thread?.dshSessionId === undefined) setError('这张卡片还没有关联的 DSH 会话')
      else post('synapse:open-canvas', { sessionId: thread.dshSessionId })
    }
    if (button.dataset.action === 'export-markdown') {
      const exportable = state.workspace
      if (exportable === null || (exportable.threads ?? []).length === 0) setError('当前工作区还没有可导出的内容')
      else downloadTextFile(outlineFileName(exportable), markdownOutline(exportable))
    }
