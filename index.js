...
export function apply(ctx, config) {
  ...
  // The DSH /api browser-trust fence does not cover /context-web routes, so this
  ...
  const api = async (req, res) => {
    try {
      ...
      if (path === '/context-web/api/reset' && req.method === 'POST') return sendJson(res, 200, await store.clearLegacy(ctx.sessions.list()))
      if (path === '/context-web/api/workspaces') {
      ...
      const workspace = /^\/context-web\/api\/workspaces\/([0-9a-f-]+)$/i.exec(path)
      if (workspace !== null) {
  ...
  ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: '/context-web/', handler: (_req, res) => { sendFile(res, 'text/html; charset=utf-8', page()) } }), 'context-web: page')
  ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: '/context-web/app.js', handler: async (_req, res) => { sendFile(res, 'text/javascript; charset=utf-8', await readFile(new URL('./app.js', import.meta.url), 'utf8')) } }), 'context-web: app')
  ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: '/context-web/styles.css', handler: async (_req, res) => { sendFile(res, 'text/css; charset=utf-8', await readFile(new URL('./styles.css', import.meta.url), 'utf8')) } }), 'context-web: styles')
  ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: '/context-web/deepseek-mark.svg', handler: async (_req, res) => { sendFile(res, 'image/svg+xml', await readFile(new URL('./deepseek-mark.svg', import.meta.url), 'utf8')) } }), 'context-web: DeepSeek mark')
  ctx.effect(() => ctx.webServer.register({ kind: 'prefix', path: '/context-web/api', handler: api }), 'context-web: api')
}
