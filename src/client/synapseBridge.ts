...
const currentSession = ctx => {
    ...
    export const apply = (ctx: any) => {
      ...
      style.textContent = '.context-web-switch{position:fixed;z-index:80;top:12px;left:50%;display:flex;gap:2px;transform:translateX(-50%);border:1px solid #d1d5db;border-radius:999px;background:rgba(255,255,255,.96);padding:3px;backdrop-filter:blur(10px)}.context-web-switch button{height:28px;border:0;border-radius:999px;background:transparent;padding:0 11px;color:#6b7280;font:600 12px Inter,system-ui,sans-serif;cursor:pointer;white-space:nowrap}.context-web-switch button:hover{background:#f3f4f6;color:#111827}.context-web-switch button.active{background:#111827;color:#fff}.context-web-switch button:focus-visible{outline:2px solid #111827;outline-offset:2px}.context-web-overlay{position:fixed;z-index:100;inset:0;background:#f5f7fa}.context-web-overlay.is-opening{visibility:hidden}.context-web-overlay[hidden]{display:none}.context-web-overlay iframe{display:block;width:100%;height:100%;border:0}'
